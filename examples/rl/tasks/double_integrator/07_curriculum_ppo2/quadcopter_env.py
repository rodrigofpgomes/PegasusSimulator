...
@dataclass
class RewardWeights:
    """Container holding the (curriculum-dependent) reward term weights."""
    constant: float = 3.0
    ep: float = 20.0
    ev: float = 2.0
    u: float = 0.001
...
class RewardCurriculumCfg:
    """Configuration of the reward-weight curriculum schedule."""
    enabled: bool = True
...
class QuadcopterEnvCfg(PegasusEnvCfg):
    ...
    lqr_q_ep: float | None = None
    lqr_q_ev: float | None = None
    lqr_r_u: float | None = None

    def __post_init__(self):
        """Dataclass post-init hook: fills derived defaults."""
        if self.lqr_q_ep is None:
            self.lqr_q_ep = self.reward_curriculum.target.ep
        if self.lqr_q_ev is None:
            self.lqr_q_ev = self.reward_curriculum.target.ev
        if self.lqr_r_u is None:
            self.lqr_r_u = self.reward_curriculum.target.u

    # Vehicle Params
    angular_damping: float = 2.0
...
class QuadcopterEnv(PegasusEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        rw_ep = torch.sum(ep ** 2, dim=1)
        rw_ev = torch.sum(ev ** 2, dim=1)
        rw_u = torch.sum(self._actions ** 2, dim=1)
    ...
    def get_reward_weights(self):
        """Returns the active reward weights for the current curriculum stage."""
    ...
    def curriculum_stage_from_env_steps(self, total_env_steps: int) -> int:
        """Maps a number of environment steps to the corresponding curriculum stage."""
        rcfg = self.cfg.reward_curriculum
        if not rcfg.enabled:
            return 0
    ...
    def set_reward_weights(self, constant: float, ep: float, ev: float, u: float):
        """Overrides the active reward weights."""
    ...
    def set_curriculum_stage(self, stage: int):
        """Sets the active curriculum stage and refreshes the reward weights."""
        rcfg = self.cfg.reward_curriculum
