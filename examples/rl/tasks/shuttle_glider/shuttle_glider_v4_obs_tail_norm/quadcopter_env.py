...
class QuadcopterEnvCfg(PegasusEnvCfg):
    ...
    w_heading:  float = 0.5    
    w_lateral:  float = 0.15  
    w_backward: float = 0.10
    w_tilt:     float = 0.10
    
    w_d_action_main: float = 1.0    # delta-action squared (action smoothness)
    w_d_action_5: float = 0.10
...
class QuadcopterEnv(PegasusEnv):
    ...
    def _get_observations(self) -> dict:
        ...
        cos_heading_error = torch.sum(forward_hat * reference_hat, dim=1)
        sin_heading_error = forward_hat[:, 0] * reference_hat[:, 1] - forward_hat[:, 1] * reference_hat[:, 0]
        ...
        if self.cfg.test_mode and self.cfg.clamp_observations_in_test:
            pos_error_b = pos_error_b.clamp(-self.cfg.obs_pos_error_limit, self.cfg.obs_pos_error_limit)
        ...
        obs = torch.cat([
            pos_error_b / self.cfg.max_pos_error_per_axis, 
            vel_b / self.cfg.max_lin_vel_per_axis, 
            vel_ref_b / self.cfg.max_lin_vel_per_axis, 
            R.reshape(self.num_envs, 9),
            ang_b / self.cfg.max_ang_vel_per_axis, 
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        pos_cost = torch.linalg.norm(pos_error_b / self.cfg.max_pos_error_per_axis, dim=1)
        vel_cost = torch.linalg.norm(vel_error_b / self.cfg.max_lin_vel_per_axis, dim=1)
        ...
        lateral_vel_cost = heading_valid.float() * (vel_b[:, 1] / self.cfg.max_lin_vel_per_axis).square()
        
        # Backward Velocity Cost: penalize moving backward
        backward_vel_cost = heading_valid.float() * (torch.relu(-vel_b[:, 0]) / self.cfg.max_lin_vel_per_axis).square()
