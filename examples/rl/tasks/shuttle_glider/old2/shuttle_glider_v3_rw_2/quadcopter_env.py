...
| Description: Shuttle_glider 31D reward ablation without velocity in the main cost.
...
Reward (static weights, no curriculum):
    r = constant - (w_pos*|pos_error| + w_d_action*|Δaction|). Termination penalty replaces the reward when the episode ends early. Cost is clipped at cost_clip=3.0.
...
class QuadcopterEnv(PegasusEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        cost = self.cfg.w_pos * pos_cost + self.cfg.w_d_action * d_action_cost
