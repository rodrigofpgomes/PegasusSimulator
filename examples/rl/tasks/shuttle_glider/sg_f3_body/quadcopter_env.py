...
| Description: Shuttle_glider 34D observation, ALL vectors in the body frame.

Observation (34 dims):
    pos_error_b              (3) - position error in the BODY frame
    vel_error_b              (3) - velocity error in the BODY frame
...
class QuadcopterEnv(PegasusEnv):
    ...
    def _get_observations(self) -> dict:
        ...
        # Position and velocity error rotated into the body frame, so that every
        # vector in the observation lives in the same frame.
        #
        # The mixed-frame observation inherited from RAPTOR is complete -- R_flat
        # is present, so body-frame quantities are recoverable in principle --
        # but recovering them means learning the bilinear map R^T . e, i.e. a
        # product of observation components. The SAC actor here is
        # Linear(obs, 64) - ReLU - Linear(64, 64) - ReLU, which represents
        # multiplicative interactions poorly.
        #
        # The bigger cost is symmetry: in the world frame the same physical
        # situation seen at a different yaw angle is a different observation, so
        # the task has to be learned once per heading. In the body frame it is
        # learned once. That matters much more for a heading-dependent objective
        # than it did for the pure hover task this environment descends from.
        pos_error_b = torch.bmm(R.transpose(1, 2), pos_error.unsqueeze(-1)).squeeze(-1)
        vel_error_b = torch.bmm(R.transpose(1, 2), vel_error.unsqueeze(-1)).squeeze(-1)

        obs = torch.cat([pos_error_b, vel_error_b, R_flat, ang_b, self._action_history_obs, rotor_speeds_norm, acc_ref_b, vel_ref_b], dim=1)
        return {"policy": obs}
