...
class QuadcopterEnv(PegasusEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        pos_cost = torch.linalg.norm(pos_error_b, dim=1)
        vel_cost = torch.linalg.norm(vel_error_b, dim=1)
        ...
        lateral_vel_cost = heading_valid.float() * torch.abs(vel_b[:, 1])
        
        # Backward Velocity Cost: penalize moving backward
        backward_vel_cost = heading_valid.float() * torch.relu(-vel_b[:, 0])
