# sg_d0_ctrl — control — diagnostics only
...
Provide the control run. This is `shuttle_glider_v3_obs_9` with the reference demand retuned to `sigma = 3.0` and nothing else changed: observation, reward, thrust cap, vehicle and the static/Langevin mixture are all left exactly as they were. Its only addition is the rotor-5 diagnostic logging, so that the new metrics exist for a policy in which none of the four candidate mechanisms has been touched.

## What changes relative to the parent (`shuttle_glider_v3_obs_9`)

- `langevin_sigma`: 9.0 -> **6.0** (retuned against the demand of the `A = 1.5 m, T = 10 s` evaluation lemniscate — see below)
- rotor-5 diagnostic logging (does not enter the reward)
...
Rotor 5 is expected to **stay useless**, i.e. `corr_thrust5_accdem` near zero. This is the baseline that makes the other five interpretable: without it, a positive result elsewhere cannot be attributed to the mechanism that variant changed rather than to the retuned sigma. It sits between the already-completed `shuttle_glider_v3_obs_4` and `_9` runs, so its tracking RMSE should land between 0.249 and 0.389; if it does not, something other than sigma changed and the rest of the family should not be trusted.
...
- maximum force: `8.549e-06 * 3500^2` = **104.7 N** (215% of the 48.8 N weight)
- force at the neutral action (`a4 = 0`, i.e. half the cap): **26.2 N** (54% of weight)
- corresponding pitch moment, with a 0.3475 m arm: **9.10 N.m** (total quadrotor authority is ~15.5 N.m)

## Observation (28D)

`[pos_error(3), vel_error(3), R_flat(9), ang_b(3), action_history(5), rotor_speeds_norm(5)]` — identical to the parent.
...
pos_cost      = ||pos_error||
vel_cost      = ||vel_error||
d_cost        = w_d_action * ||d_action||          # single norm, 5 dims
...
In this variant `w_tilt = 0.0`, so the tilt term is computed and logged but **does not affect the reward**.
...
```bash
isaac_run examples/rl/train.py --task shuttle_glider/sg_d0_ctrl \
...
The preset in `agents/sac_cfg.py` needs no edit: `Policy` and `Critic` build their layers from `observation_space` (`nn.Linear(self.num_observations, 64)`), so the 28D observation is picked up automatically.
...
|---|---|---|---|---|---|
| `sg_d0_ctrl` **<-- this one** | 28 | `None` | `0.0` | 3500 | `Shuttle_glider` |
...
| `sg_d4_com` | 31 | `0.10` | `2.0` | 1000 | `Shuttle_glider_com` |
