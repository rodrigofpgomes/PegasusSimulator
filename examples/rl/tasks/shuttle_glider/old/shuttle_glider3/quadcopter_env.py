...
| Description: Legacy Shuttle_glider 5-actuator hover/pre-training task.

Observation (23 dims):
    pos_error                (3) - position error in world frame (pos - goal / reference)
    vel_error                (3) - velocity error in world frame
    e_R                      (3) - geometric attitude error vector
    ori_cost.unsqueeze(1)    (1) - scalar orientation cost
...
Reward (static weights, no curriculum):
    r = constant - (w_pos*|pos_error| + w_ori*ori_cost + w_d_action*|Δaction|). Termination penalty replaces the reward when the episode ends early.
...
class QuadcopterEnvCfg(PegasusEnvCfg):
    ...
    w_ori:      float = 0.3    # orientation cost (1 - qw^2)
    w_d_action: float = 1.0    # delta-action squared (action smoothness)
    constant:   float = 1.5
    termination_penalty: float = 100.0

    # --- heading para aproveitar o puller do glider (NOVO) ---
    yaw_speed_threshold: float = 0.3   # [m/s] abaixo disto o heading e' indefinido (pontas do "8") -> segura o yaw atual
    max_yaw_error:       float = 0.6   # [rad] limite de quao a' frente o heading de referencia pode estar do yaw atual
...
class QuadcopterEnv(PegasusEnv):
    ...
    def setup(self):
        ...
        self._episode_sums = {
            ...
            for k in ("pos", "ori", "d_action", "total")
        }
    ...
    def _get_observations(self) -> dict:
        ...
        yaw_ref = self._desired_yaw(current_yaw)

        Rd_ref = self.build_Rd_from_acc_yaw(acc_w=goal_acc, yaw=yaw_ref, g=9.81)
        e_R, ori_cost = self.attitude_error_geometric(R, Rd_ref)
        ...
        obs = torch.cat([pos_error, vel_error, e_R, ori_cost.unsqueeze(1), ang_b, self._action_history_obs, rotor_speeds_norm], dim=1)
        return {"policy": obs}
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        R = quaternion_to_matrix(quat)
        _, ori_cost = self.attitude_error_geometric(R, Rd_ref)
        ...
        self._episode_sums["ori"] += -self.cfg.w_ori * ori_cost
    ...
    def attitude_error_geometric(self, R: torch.Tensor, Rd: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Computes the geometric attitude error on SO(3).

        Args:
            R:  Current rotation matrix, shape (N, 3, 3)
            Rd: Desired rotation matrix, shape (N, 3, 3)

        Returns:
            e_R:      Geometric attitude error vector, shape (N, 3)
            ori_cost: Scalar attitude cost Psi = 0.5 * tr(I - Rd^T R), shape (N,)
        """
        Rt = R.transpose(1, 2)
        Rdt = Rd.transpose(1, 2)

        E = torch.bmm(Rdt, R)  # Rd^T R

        # e_R = 0.5 * vee(Rd^T R - R^T Rd)
        skew = E - E.transpose(1, 2)
        ...
        ori_cost = 0.5 * (3.0 - trace_E)

        # Numerical guard: theoretically in [0, 2]
        ori_cost = ori_cost.clamp(min=0.0, max=2.0)
    ...
    def build_Rd_from_acc_yaw(self, acc_w: torch.Tensor, yaw: torch.Tensor, g: float = 9.81) -> torch.Tensor:
        """Builds the reference rotation matrix Rd from the desired acceleration and yaw (RAPTOR pre-training formulation)."""
        # acc_w is the desired translational acceleration in world frame, excluding gravity
        # yaw is the desired yaw angle in radians, shape (N,)

        gravity = torch.zeros_like(acc_w)
        gravity[:, 2] = g

        thrust_w = acc_w + gravity

        z_b = thrust_w / thrust_w.norm(dim=1, keepdim=True).clamp_min(1e-8) # Desired body z-axis
        x_c = torch.stack([yaw.cos(), yaw.sin(), torch.zeros_like(yaw)], dim=1)  # Yaw direction in world frame
        y_b = torch.cross(z_b, x_c, dim=1)  # Desired body y-axis
        y_b = y_b / y_b.norm(dim=1, keepdim=True).clamp_min(1e-8)
        x_b = torch.cross(y_b, z_b, dim=1)  # Desired body x-axis

        Rd = torch.stack([x_b, y_b, z_b], dim=2)  # (N, 3, 3)
        return Rd

    @staticmethod
    def _yaw_from_quat(quat: torch.Tensor) -> torch.Tensor:
        """Yaw (rotacao em torno do Z do mundo) a partir do quaterniao (N,4) em ordem w,x,y,z."""
        w, x, y, z = quat[:, 0], quat[:, 1], quat[:, 2], quat[:, 3]
        return torch.atan2(2.0 * (w * z + x * y), 1.0 - 2.0 * (y * y + z * z))

    def _desired_yaw(self, current_yaw: torch.Tensor) -> torch.Tensor:
        """Heading desejado = direcao da velocidade de referencia.

        - Deadband a baixa velocidade (pontas do "8", onde a direcao e' indefinida): segura o yaw atual.
        - Limitado a +/- max_yaw_error rad face ao yaw atual (carrot-chasing): mantem sempre uma
        pressao para guinar, mas nunca exige uma guinada instantanea impossivel.
        """
        gv = self.reset_manager.goal_vel                      # (N,3) mundo
        speed_xy = torch.linalg.norm(gv[:, :2], dim=1)
        target = torch.atan2(gv[:, 1], gv[:, 0])
        target = torch.where(speed_xy > self.cfg.yaw_speed_threshold, target, current_yaw)
        # erro angular envolto em [-pi, pi]
        err = torch.atan2(torch.sin(target - current_yaw), torch.cos(target - current_yaw))
        err = err.clamp(-self.cfg.max_yaw_error, self.cfg.max_yaw_error)
        return current_yaw + err