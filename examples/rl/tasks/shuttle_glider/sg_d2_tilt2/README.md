# sg_d2_tilt — make tilt expensive — at the right scale
...
Attack the actual reason rotor 5 loses: the quadrotor gets horizontal force by tilting, and tilting costs nothing in the 28D cost. This env puts a price on tilt. Critical scale note: the formulation used is `acos(R[2,2])` in radians, **not** `1 - cos(theta)`.

## What changes relative to the parent (`shuttle_glider_v3_obs_9`)

- everything `sg_d1_obs` changes
- `w_tilt`: 0.0 -> **2.0**, with `tilt_cost = acos(R[2,2])`
...
The first variant with a real chance. The earlier attempt used `w_tilt = 0.10` on `1 - cos(theta)`, which at 7.4 degrees is worth 0.10 * 0.00833 = 0.00083 against a `pos_cost` of ~0.5 — a ratio of 1 to 600. That was a non-existent term, not a weak incentive. With `acos`, 7.4 degrees gives 0.129 rad, so `w_tilt = 2.0` costs 0.26: **313x** the previous value, and finally comparable to `pos_cost`.
...
```bash
isaac_run examples/rl/train.py --task shuttle_glider/sg_d2_tilt \
...
| `sg_d2_tilt` **<-- this one** | 31 | `0.10` | `2.0` | 3500 | `Shuttle_glider` |
