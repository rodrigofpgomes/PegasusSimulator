...
class QuadcopterEnvCfg(PegasusEnvCfg):
    ...
    disable_rotor_5: bool = False
    use_puller_slipstream: bool = False
    ...
    w_vel:      float = 0.5    # velocity cost
    
    w_heading:  float = 0.75    
    w_lateral:  float = 0.25  
    w_backward: float = 0.10
    w_tilt:     float = 0.10
    
    w_d_action_main: float = 0.5    # delta-action squared (action smoothness)
    ...
    trajectory_mixture_langevin_prob: float = 0.9
    
    langevin_gamma: float = 1.0
    langevin_omega: float = 2.0
    langevin_sigma: float = 2.5
    ...
    # Observation scales
    obs_pos_scale: float = 1.0
    obs_vel_scale: float = 2.0
    obs_ref_vel_scale: float = 1.0
    obs_ang_scale: float = 35.0
    
    # Reward scales
    reward_pos_scale: float = 1.0
    reward_vel_scale: float = 1.0
...
class QuadcopterEnv(PegasusEnv):
    ...
    def setup(self):
        ...
        self._episode_sums = {
            ...
            for k in ("pos", "vel", "d_action_main", "d_action_5", "heading", "lateral", "backward", "tilt", "total")
        }
    ...
    def _apply_action(self):
        ...
        if self.cfg.disable_rotor_5:
            omega[:, self.cfg.action_space-1] = 0.0  # zero out the fifth rotor
    ...
    def _get_observations(self) -> dict:
        ...
        pos_ref_w = self.reset_manager.goal_pos
        vel_ref_w = self.reset_manager.goal_vel
        
        pos_error_w = pos_w - pos_ref_w
        
        # Transform world-frame errors to body frame for the policy observation
        pos_error_b = (R.transpose(1, 2) @ pos_error_w.unsqueeze(-1)).squeeze(-1)
        
        vel_b = (R.transpose(1, 2) @ vel_w.unsqueeze(-1)).squeeze(-1)
        vel_ref_b = (R.transpose(1, 2) @ vel_ref_w.unsqueeze(-1)).squeeze(-1)
        
        vel_error_b = vel_b - vel_ref_b

        # Calculate the heading and lateral errors
        ref_vel_xy = self.reset_manager.goal_vel[:, :2]
        ref_speed_xy = torch.linalg.vector_norm(ref_vel_xy, dim=1)
        heading_valid = ref_speed_xy > 0.15
        
        # Longitudinal axis in body frame (forward direction)
        forward_xy = R[:, :2, 0]
        forward_hat = forward_xy / torch.linalg.vector_norm(forward_xy, dim=1, keepdim=True).clamp_min(1e-6)
        reference_hat = ref_vel_xy / ref_speed_xy.unsqueeze(-1).clamp_min(1e-6)

        # Heading error: angle between forward direction and reference velocity
        cos_heading_error = torch.sum(forward_hat * reference_hat, dim=1).clamp(-1.0, 1.0)
        sin_heading_error = (forward_hat[:, 0] * reference_hat[:, 1] - forward_hat[:, 1] * reference_hat[:, 0]).clamp(-1.0, 1.0)

        cos_heading_error = torch.where(heading_valid, cos_heading_error, torch.ones_like(cos_heading_error))
        sin_heading_error = torch.where(heading_valid, sin_heading_error, torch.zeros_like(sin_heading_error))
        
        # Clamp observations only during test/evaluation
        if self.cfg.test_mode and self.cfg.clamp_observations_in_test:
            pos_error_b = pos_error_b.clamp(-self.cfg.max_pos_error_per_axis, self.cfg.max_pos_error_per_axis)
        ...
        obs = torch.cat([
            pos_error_b / self.cfg.obs_pos_scale, 
            vel_error_b / self.cfg.obs_vel_scale, 
            vel_ref_b / self.cfg.obs_ref_vel_scale, 
            R.reshape(self.num_envs, 9),
            ang_b / self.cfg.obs_ang_scale, 
            self._action_history_obs, 
            rotor_speeds_norm,
            cos_heading_error.unsqueeze(-1),
            sin_heading_error.unsqueeze(-1),
            heading_valid.float().unsqueeze(-1),
        ], dim=1)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # Initial costs
        vel_error_b = vel_b - vel_ref_b
        pos_cost = torch.linalg.norm(pos_error_b / self.cfg.reward_pos_scale, dim=1)
        vel_cost = torch.linalg.norm(vel_error_b / self.cfg.reward_vel_scale, dim=1)

        # Calculate heading and validity
        ref_vel_xy = vel_ref_w[:, :2]
        ref_speed_xy = torch.linalg.vector_norm(ref_vel_xy, dim=1)
        heading_valid = ref_speed_xy > 0.15
        ...
        # Heading Cost: penalize deviation from the reference heading
        heading_cost = heading_valid.float() * (1.0 - cos_heading_error)
        
        # Lateral Velocity Cost: penalize lateral movement
        lateral_vel_cost = heading_valid.float() * (vel_b[:, 1] / self.cfg.reward_vel_scale).square()
        
        # Backward Velocity Cost: penalize moving backward
        backward_vel_cost = heading_valid.float() * (torch.relu(-vel_b[:, 0]) / self.cfg.reward_vel_scale).square()
        
        tilt_cost = 1.0 - R[:, 2, 2].clamp(-1.0, 1.0)
        
        d_action_main_cost = torch.linalg.vector_norm(d_action[:, :4], dim=1)

        d_action_5_cost = torch.abs(d_action[:, 4])
        
        cost = (
            self.cfg.w_pos * pos_cost 
            + self.cfg.w_vel * vel_cost 
            + self.cfg.w_heading * heading_cost
            + self.cfg.w_lateral * lateral_vel_cost
            + self.cfg.w_backward * backward_vel_cost
            + self.cfg.w_tilt * tilt_cost
            + self.cfg.w_d_action_main * d_action_main_cost
        ...
        died = (pos_error_w.abs() > self.cfg.max_pos_error_per_axis).any(dim=1)
        ...
        self._episode_sums["heading"] += -self.cfg.w_heading * heading_cost
        self._episode_sums["lateral"] += -self.cfg.w_lateral * lateral_vel_cost
        self._episode_sums["backward"] += -self.cfg.w_backward * backward_vel_cost
        self._episode_sums["tilt"] += -self.cfg.w_tilt * tilt_cost
        self._episode_sums["d_action_main"] += -self.cfg.w_d_action_main * d_action_main_cost
    ...
    def _reset_idx(self, env_ids: torch.Tensor):
        ...
        reset_action = self.backend._vehicle._thrusters._reset_rotor_norm[env_ids]
    ...
    def vertical_tail_wrench(self, v_com_b, omega_b, wind_b=None, rho=1.225):
        ...
        if self.cfg.use_puller_slipstream and not self.cfg.disable_rotor_5:
