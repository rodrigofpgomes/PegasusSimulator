# sg_d3_cap — size the cap — action conditioning
...
Fix the problem that most likely dominates all the others: with `max_rotor_velocity[4] = 3500`, the affine action mapping means `a4 = 0` (the neutral action) produces half the cap, i.e. 26.2 N — 54% of vehicle weight — and 9.1 N.m of pitch, against roughly 15.5 N.m of total quadrotor authority. The policy has no choice: to stabilise it must push `a4` to the floor, and near the floor the gain `dF/da` drops from ~52 to ~5 N per unit action. The learning signal vanishes.

## What changes relative to the parent (`shuttle_glider_v3_obs_9`)

- everything `sg_d2_tilt` changes
- `max_rotor_velocity[4]`: 3500 -> **1000**
...
The variant most likely to work, and therefore the first one to run. The neutral action goes from 26.2 N (54% of weight) to 2.1 N (4%), and the rotor starts operating in the region where `dF/da` is useful. A cap of 3500 was not more authority, it was worse conditioning. **If d2 fails and d3 works, the problem was the action parametrisation — not the physics and not the reward.**
...
```bash
isaac_run examples/rl/train.py --task shuttle_glider/sg_d3_cap \
...
| `sg_d3_cap` **<-- this one** | 31 | `0.10` | `2.0` | 1000 | `Shuttle_glider` |
