# sg_d5_hard — strong incentive — tilt clearly dominant
...
Saturate the incentive in order to distinguish "the incentive was too weak" from "the incentive is in the wrong place". If d3 does not move rotor 5 at `w_tilt = 2.0`, this env quadruples the weight.
...
At 7.4 degrees the tilt cost becomes 1.03, i.e. **larger than the typical `pos_cost`**. There is a real risk of pathology: the policy may prefer to stay level and simply not track the reference, accepting the position error to avoid the attitude cost. That is why `Metrics/die_pos` and `rmse_pos` matter as much as the correlation here. If d5 uses rotor 5 but with a much worse `rmse_pos`, the right weight lies between 2 and 8 and a sweep is needed.
...
With `w_tilt = 8.0` and `acos`, a 7.4 degree tilt (0.129 rad) costs **1.03**, against a typical `pos_cost` of ~0.5.
...
```bash
isaac_run examples/rl/train.py --task shuttle_glider/sg_d5_hard \
...
| `sg_d5_hard` **<-- this one** | 31 | `0.10` | `8.0` | 1000 | `Shuttle_glider` |
