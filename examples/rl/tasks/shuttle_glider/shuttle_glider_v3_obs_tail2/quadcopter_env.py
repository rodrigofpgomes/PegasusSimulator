...
class QuadcopterEnv(PegasusEnv):
    ...
    def _apply_action(self):
        ...
        external_forces, external_torques = self.vertical_tail_wrench(vel_b.squeeze(-1), ang_b, wind_b.squeeze(-1))
    ...
    def vertical_tail_wrench(
        ...
        p_com = torch.tensor(
            [-0.06945947, 0.0, 0.10253066],
            device=device,
            dtype=dtype,
        )

        p_vtail = torch.tensor(
            [-0.848256, 0.0, -0.093542],
            device=device,
            dtype=dtype,
        )
        ...
        airspeed_xy = torch.linalg.vector_norm(
            v_xy, dim=-1, keepdim=True
        )
        ...
        # +beta: veículo desloca-se na direção +Y relativamente ao ar.
        beta = torch.atan2(vy, vx)

        # Parâmetros iniciais da cauda vertical.
        S_vtail = 0.022078
        CL_beta = 3.0       # rad^-1
        CL_max = 0.8
        CD0 = 0.03
        ...
        # Coeficiente lateral com saturação suave.
        cl_linear = CL_beta * beta
        cl = CL_max * torch.tanh(cl_linear / CL_max)

        # O modelo de lift da cauda é relevante em voo frontal.
        forward_gate = torch.sigmoid((vx - 0.5) / 0.15)
        cl = cl * forward_gate

        # Drag de perfil + induzido + crossflow a sideslip elevado.
        cd = (
            CD0
            + induced_k * cl.square()
            + CD_crossflow * torch.sin(beta).square()
        )

        dynamic_pressure = (
            0.5
            * rho
            * airspeed_xy.squeeze(-1).square()
        )

        # Eixo vertical da superfície.
        span_axis = torch.zeros_like(v_xy)
        span_axis[..., 2] = 1.0

        # Para velocidade +X:
        # cross(+X, +Z) = -Y.
        # Portanto beta positivo produz uma força restauradora em -Y.
        side_force_hat = F.normalize(
            torch.cross(v_hat, span_axis, dim=-1),
            dim=-1,
            eps=1e-6,
        )

        force_scale = (
            dynamic_pressure * S_vtail
        ).unsqueeze(-1)
        ...
        # Evitar direções numéricas instáveis perto de velocidade zero.
        ...
        # Momento relativamente ao COM.
        torque_b = torch.cross(
            r_vtail,
            force_b,
            dim=-1,
        )
