# sg_d1_obs — information + stop penalising modulation
...
Give the policy the information it is missing about *when* to use rotor 5, and stop punishing it for modulating it. The reference acceleration projected into the body frame (`acc_ref_b`) is exactly the signal that says "I am about to need horizontal force"; without it the policy has to infer that from the tracking error, which arrives too late to act on.

## What changes relative to the parent (`shuttle_glider_v3_obs_9`)

- `observation_space`: 28 -> **31** (adds `acc_ref_b`, 3 components)
- `w_d_action_5`: `None` -> **0.10** (splits the smoothness cost into `||d_action[:, :4]||*1.0 + |d_action[4]|*0.10`)
- `langevin_sigma`: 9.0 -> 6.0, as in `sg_d0_ctrl`
...
Small or no effect on its own. `w_d_action = 1.0` across all 5 dimensions penalised moving rotor 5 as heavily as moving lift, which is absurd, but fixing it *removes a penalty* — it does not create an incentive. Without making tilt expensive, tilting stays free and keeps winning.
...
```bash
isaac_run examples/rl/train.py --task shuttle_glider/sg_d1_obs \
...
| `sg_d1_obs` **<-- this one** | 31 | `0.10` | `0.0` | 3500 | `Shuttle_glider` |
