...
class QuadcopterEnvCfg(PegasusEnvCfg):
    ...
    def __init__(self, **kwargs):
        # PegasusEnvCfg e' um @dataclass SEM __post_init__, logo o __init__
        # gerado pelo dataclasses nunca chama o nosso. E o _load_env() do
        # skrl_agent_controller faz apenas QuadcopterEnvCfg(), sem post-init
        # explicito. Sem este __init__, action_space/observation_space ficavam
        # nos defaults de classe (5/28) e a condicao "removed" (4 acoes, 26
        # obs) tentaria carregar o checkpoint com as dimensoes erradas.
    ...
    def __post_init__(self):
        ...
        # __post_init__ passa a correr mais do que uma vez (uma no __init__,
        # outra depois de cfg_E3 mexer no reward_mode). Sem memorizar o valor
        # declarado, a segunda passagem via o None ja' resolvido para False e
        # o env de heading ficava sem as 3 observacoes de heading.
        if not hasattr(self, "_use_heading_obs_declared"):
            self._use_heading_obs_declared = self.use_heading_obs
        if self._use_heading_obs_declared is None:
            self.use_heading_obs = self.reward_mode == "heading"
        else:
            self.use_heading_obs = bool(self._use_heading_obs_declared)
