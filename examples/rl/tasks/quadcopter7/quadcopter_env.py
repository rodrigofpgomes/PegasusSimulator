...
from .lyapunov_network import lyapunov_network
...
class QuadcopterEnvCfg(PegasusEnvCfg):
    ...
    # Reward Scales
    lin_vel_error_reward_scale: float = 0.4
    position_error_reward_scale: float = 1.5
    lyapunov_reward_scale: float = 3.0

    K: List[float] = field(default_factory=lambda: [1.0, 1.0, 1.5])
...
class QuadcopterEnv(PegasusEnv):
    ...
    def setup(self):
        ...
        self._lyapunov_net = lyapunov_network(observation_space=self.cfg.observation_space, action_space=self.cfg.action_space, device=self.device, checkpoint_path="best_agent.pt")
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        K = torch.tensor(self.cfg.K, device=self.device)

        #V = 0.5 * (
        #    torch.sum((1 + torch.square(K)) * torch.square(ep), dim=1) +  # (1 + k_i^2) * ep_i^2
        #    torch.sum(torch.square(ev), dim=1) +
        #    2 * torch.sum(K * ep * ev, dim=1)       # 2 * k_i * ep_i * ev_i
        #)

        V = self._lyapunov_net.infer_value(torch.cat([ep, ev], dim=-1))

        rewards = {
            "ev": - rw_ev * self.cfg.lin_vel_error_reward_scale * self.step_dt,
            "ep": - rw_ep * self.cfg.position_error_reward_scale * self.step_dt,
            "v" : (V-self._prev_v) * self.cfg.lyapunov_reward_scale * self.step_dt,
        }
    ...
    def _reset_idx(self, env_ids: torch.Tensor):
        ...
        ep = self._goal_pos[env_ids] - self.reset_manager._init_pos[env_ids]
        ev = torch.zeros((env_ids.numel(), 3), device=self.device)

        self._prev_v[env_ids] = self._lyapunov_net.infer_value(torch.cat([ep, ev], dim=-1))
