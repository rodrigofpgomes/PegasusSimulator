...
Port do erro geométrico de atitude para `Shuttle` com `w_ori = 0.1`.
...
## Observações
- Dimensão configurada: `21`.
- Vetor `policy`: `[pos_error, vel_error, e_R, ori_cost.unsqueeze(1), ang_b, self._action_history_obs, rotor_speeds_norm]`.
...
- `terminated` = `terminated |= (vel_error.abs() > self.cfg.max_lin_vel_per_axis).any(dim=1)`
...
- Usa erro geométrico de atitude `e_R` e custo `Psi`, com `ori_cost` limitado a `[0, 2]`.
- A atitude desejada é construída a partir da aceleração de referência e yaw/heading.
