...
| Description: Shuttle pre-training with geometric attitude error and higher w_ori.
...
Observation (21 dims):
    ...
    self._action_history_obs (4) - previous normalised motor command (ActionHistory length=1)
    rotor_speeds_norm        (4) - actual rotor speeds normalised to [-1, 1]

Action (4 dims):
    Normalised rotor velocity in [-1, 1], mapped to [min_w, max_w] = [0, 0, 0, 0]..[1400, 1400, 1400, 1400].
...
class QuadcopterEnvCfg(PegasusEnvCfg):
    ...
    vehicle_physics_cfg: Any = field(default_factory=lambda: _SHUTTLE_PHYSICS_CFG)
...
class QuadcopterEnv(PegasusEnv):
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        terminated |= (vel_error.abs() > self.cfg.max_lin_vel_per_axis).any(dim=1)
