...
class QuadcopterEnvCfg(PegasusEnvCfg):
    ...
    w_vel:      float = 0.2    # linear velocity squared error
...
class QuadcopterEnv(PegasusEnv):
    ...
    def _get_observations(self) -> dict:
        ...
        obs = torch.cat([pos_error, vel_error, R_err_flat, ang_b, self._action_history_obs, rotor_speeds_norm], dim=1)
        return {"policy": obs}
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        cost = self.cfg.w_pos * pos_cost + self.cfg.w_vel * vel_cost + self.cfg.w_ori * ori_cost + self.cfg.w_d_action * d_action_cost
