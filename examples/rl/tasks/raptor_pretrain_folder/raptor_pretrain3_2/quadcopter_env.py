...
class QuadcopterEnv(PegasusEnv):
    ...
    def _get_observations(self) -> dict:
        ...
        R_err = torch.bmm(Rd_ref.transpose(1, 2), quaternion_to_matrix(quat))
        ...
        obs = torch.cat([pos_error, R_err_flat, vel_error, ang_b, self._action_history_obs, rotor_speeds_norm], dim=1)
        return {"policy": obs}
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        trace_R_err = R_err.diagonal(dim1=-2, dim2=-1).sum(dim=-1)
        trace_R_err = trace_R_err.clamp(-1.0, 3.0)

        ori_cost = 0.5 * (3.0 - trace_R_err)
