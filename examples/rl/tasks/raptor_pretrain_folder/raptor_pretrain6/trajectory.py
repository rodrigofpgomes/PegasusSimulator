...
class RaptorLikeTrajectory:
    ...
    def __init__(
        ...
        self.jerk_traj = torch.zeros(num_envs, episode_steps, 3, device=device)
    ...
    def reset(self, env_ids: torch.Tensor, centers: torch.Tensor):
        ...
        for t in range(1, self.episode_steps):
            ...
            self.jerk_traj[langevin_ids, t] = (self.acc_traj[langevin_ids, t] - self.acc_traj[langevin_ids, t - 1]) / self.dt

        self.acc_traj[langevin_ids, 0] = self.acc_traj[langevin_ids, 1]
        self.jerk_traj[langevin_ids, 0] = self.jerk_traj[langevin_ids, 1]
    ...
    def current(self, env_ids: torch.Tensor | None = None, step_ids: torch.Tensor | None = None):
        ...
        jerk = self.jerk_traj[env_ids, index]
