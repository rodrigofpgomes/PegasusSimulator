...
| Description: Single-vehicle hover task replicating the RAPTOR pre-training setup.

Observation (26 dims):
    pos_error  (3)  - position error in world frame (pos - goal)
    R_err_flat (9) - attitude error matrix Rdᵀ R, row-major
    vel_w      (3)  - linear velocity in world frame
    ang_vel_b  (3)  - angular velocity in body frame
    last_act   (4)  - previous normalised motor command (ActionHistory length=1)
    rotor_norm (4)  - actual rotor speeds normalised to [-1, 1] (RotorSpeeds obs)

Action (4 dims):
    Normalised rotor velocity in [-1, 1], mapped to [min_w, max_w].

Reward (static weights, no curriculum — matching RAPTOR pre-training exactly):
    r = constant - (w_pos*|pos_error|² + w_vel*|vel_w|² + w_act*|action|²)
    Termination penalty replaces the reward when the episode ends early.
...
# Iris real parameters (PX4/Gazebo iris.sdf):
#   kf = 8.54858e-6, moment_constant = 0.016 -> km = 0.016 * kf = 1.368e-7
_IRIS_PHYSICS_CFG = {
    "num_rotors": 4,
    "rotor_constant": [8.54858e-6, 8.54858e-6, 8.54858e-6, 8.54858e-6],
    "rolling_moment_coefficient": [1.3677736e-7, 1.3677736e-7, 1.3677736e-7, 1.3677736e-7],
    "rot_dir": [-1, -1, 1, 1],
...
class QuadcopterEnvCfg(PegasusEnvCfg):
    ...
    vehicle_physics_cfg: Any = field(default_factory=lambda: _IRIS_PHYSICS_CFG)
    ...
    w_ang_vel:  float = 0.05    # angular velocity squared error
...
class QuadcopterEnv(PegasusEnv):
    ...
    def _sync_trajectory_reference(self, env_ids: torch.Tensor | None = None):
        ...
        pos_ref, vel_ref, acc_ref, jerk_ref = self._trajectory.current(env_ids, step_ids)
        ...
        self.reset_manager._goal_jerk[env_ids] = jerk_ref
    ...
    def _get_observations(self) -> dict:
        ...
        goal_jerk = self.reset_manager.goal_jerk
        #goal_acc = self.reset_manager.goal_acc - 2.0 * pos_error - 1.2 * vel_error

        yaw_ref = torch.zeros(goal_acc.shape[0], device=self.device, dtype=goal_acc.dtype)
        ang_ref, Rd_ref = self.compute_desired_omega(acc_ref=goal_acc, jerk_ref=goal_jerk, yaw=yaw_ref, g=9.81)

        R = quaternion_to_matrix(quat)

        R_err = torch.bmm(Rd_ref.transpose(1, 2), R)  # Rdᵀ R
        R_err_flat = R_err.reshape(self.num_envs, 9)

        R_rel = torch.bmm(R.transpose(1, 2), Rd_ref)

        ang_ref_body = torch.bmm(R_rel, ang_ref.unsqueeze(2)).squeeze(2)

        ang_err = ang_b - ang_ref_body
        ...
        obs = torch.cat([pos_error, vel_error, R_err_flat, ang_err, self._action_history_obs, rotor_speeds_norm], dim=1)
        return {"policy": obs}
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        cos_b3 = torch.sum(b3 * b3_des, dim=1).clamp(-1.0, 1.0)
        ori_cost = 1.0 - cos_b3
        ...
        ang_cost = torch.linalg.norm(ang_err, dim=1)
        ...
        cost = self.cfg.w_pos * pos_cost + self.cfg.w_ori * ori_cost + self.cfg.w_ang_vel * ang_cost + self.cfg.w_d_action * d_action_cost
    ...
    def compute_desired_omega(self, acc_ref: torch.Tensor, jerk_ref: torch.Tensor, yaw: torch.Tensor, g: float = 9.81) -> tuple[torch.Tensor, torch.Tensor]:
        """Computes the desired angular velocity omega_d and reference rotation matrix Rd from the desired acceleration, jerk, and yaw."""
        ...
        device = acc_ref.device
        N = acc_ref.shape[0]

        I = torch.eye(3, device=device).unsqueeze(0).repeat(N, 1, 1)

        # b3d
        gravity = torch.zeros_like(acc_ref)
        gravity[:, 2] = g

        fd = acc_ref + gravity

        fd_norm = fd.norm(dim=1, keepdim=True).clamp_min(1e-8)

        b3 = fd / fd_norm

        # db3
        P = I - torch.bmm(b3.unsqueeze(2), b3.unsqueeze(1))

        db3 = torch.bmm(P, jerk_ref.unsqueeze(2)).squeeze(2)
        db3 = db3 / fd_norm

        # b1c
        b1c = torch.stack([torch.cos(yaw), torch.sin(yaw), torch.zeros_like(yaw)], dim=1)

        # yaw_rate = 0
        db1c = torch.zeros_like(b1c)

        # b2
        c = torch.cross(b3, b1c, dim=1)

        c_norm = c.norm(dim=1, keepdim=True).clamp_min(1e-8)

        b2 = c / c_norm

        dc = torch.cross(db3, b1c, dim=1)

        P2 = I - torch.bmm(b2.unsqueeze(2), b2.unsqueeze(1))

        db2 = torch.bmm(P2, dc.unsqueeze(2)).squeeze(2)
        db2 = db2 / c_norm

        # b1
        b1 = torch.cross(b2, b3, dim=1)
        db1 = torch.cross(db2, b3, dim=1) + torch.cross(b2, db3, dim=1)

        # Rd
        Rd = torch.stack([b1, b2, b3], dim=2)

        Rdot = torch.stack([db1, db2, db3], dim=2)
        omega_hat = torch.bmm(Rd.transpose(1, 2), Rdot)

        omega_d = vee(omega_hat)
