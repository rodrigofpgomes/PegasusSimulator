...
from .trajectory_lissajous import LissajousTrajectory
...
class QuadcopterEnvCfg(PegasusEnvCfg):
    ...
    lissajous_period: float = 10.0
    lissajous_z_ref: float = 1.0
    lissajous_amplitude: float = 1.5
...
class QuadcopterEnv(PegasusEnv):
    ...
    def setup(self):
        ...
        if self.cfg.use_raptor_trajectory:
            self._trajectory = LissajousTrajectory(
                num_envs=self.num_envs,
                episode_steps=self.max_episode_length,
                dt=self.cfg.sim_dt * self.cfg.decimation,
                device=self.device,
                period=self.cfg.lissajous_period,
                z_ref=self.cfg.lissajous_z_ref,
                amplitude=self.cfg.lissajous_amplitude,
                center=self.backend._vehicle._init_pos.to(device=self.device, dtype=torch.float32),
    ...
    def _sync_trajectory_reference(self, env_ids: torch.Tensor | None = None):
        ...
        pos_ref, vel_ref = self._trajectory.current(env_ids, step_ids)
    ...
    def _get_observations(self) -> dict:
        ...
        obs = torch.cat([pos_error, R_flat, vel_error, ang_b, self._action_history_obs, rotor_speeds_norm], dim=1)
        return {"policy": obs}
