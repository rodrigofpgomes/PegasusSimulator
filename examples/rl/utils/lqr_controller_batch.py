...
from pegasus.simulator.logic.transforms import quaternion_to_matrix, quaternion_apply, quaternion_invert
...
class LQRBackend(Backend):
    ...
    def __init__(self, n_vehicles: int, P: torch.Tensor, K: torch.Tensor, reset_manager=None, action_mode: str = "rotor_velocity", inner_loop: bool = True):
        """Initializes the backend with vehicle count, action mode, and control loop preference."""        
        super().__init__(config=None)

        self.cfg: LQRBackendCfg = LQRBackendCfg()
    ...
    def setup(self, reset_manager):
        """Sets up the LQR backend with the reset manager."""
    ...
    def start(self):
        ...
        self._P = self._P.to(device=self._device, dtype=torch.float32)
        self._K = self._K.to(device=self._device, dtype=torch.float32)
    ...
    def update(self, dt: float):
        ...
        pos = self._state_cache[:, 0:3]
        vel = self._state_cache[:, 3:6]
        ...
        x = torch.cat([ep, ev], dim=1)

        u_lqr = x @ self._K.T

        g_force = torch.tensor(
            [0.0, 0.0, self.cfg.drone_mass * self.cfg.gravity],
            dtype=torch.float32,
            device=self._device,
        ).unsqueeze(0)

        F_des = self.cfg.drone_mass * u_lqr + g_force

        self._forces[:, self._vehicle.body_index, :] = quaternion_apply(quaternion_invert(quat), F_des)
        self._torques[:, self._vehicle.body_index, :] = - self.cfg.angular_damping * ang_vel_b
