...
class NonlinearControllerBatch(Backend):
    ...
    def pd(self, t, s, reverse=False):
        ...
        x = t
        z = 1 / s * torch.exp(-0.5 * torch.pow(t/s, 2)) + 1.0
        y = 1 / s * torch.exp(-0.5 * torch.pow(t/s, 2))

        if reverse == True:
            y = -1 / s * torch.exp(-0.5 * torch.pow(t/s, 2)) + 4.5
    ...
    def d_pd(self, t, s, reverse=False):
        ...
        x = 1.0
        y = -(t * torch.exp(-torch.pow(t,2)/(2*torch.pow(s,2))))/torch.pow(s,3)
        z = -(t * torch.exp(-torch.pow(t,2)/(2*torch.pow(s,2))))/torch.pow(s,3)
    ...
    def dd_pd(self, t, s, reverse=False):
        ...
        x = 0.0
        y = (torch.pow(t,2)*torch.exp(-torch.pow(t,2)/(2*torch.pow(s,2))))/torch.pow(s,5) - torch.exp(-torch.pow(t,2)/(2*torch.pow(s,2)))/torch.pow(s,3)
        z = (torch.pow(t,2)*torch.exp(-torch.pow(t,2)/(2*torch.pow(s,2))))/torch.pow(s,5) - torch.exp(-torch.pow(t,2)/(2*torch.pow(s,2)))/torch.pow(s,3)

        if reverse == True:
            y = torch.exp(-torch.pow(t,2)/(2*torch.pow(s,2)))/torch.pow(s,3) - (torch.pow(t,2)*torch.exp(-torch.pow(t,2)/(2*torch.pow(s,2))))/torch.pow(s,5)
    ...
    def ddd_pd(self, t, s, reverse=False):
        ...
        x = 0.0
        y = (3*t*torch.exp(-torch.pow(t,2)/(2*torch.pow(s,2))))/torch.pow(s,5) - (torch.pow(t,3)*torch.exp(-torch.pow(t,2)/(2*torch.pow(s,2))))/torch.pow(s,7)
        z = (3*t*torch.exp(-torch.pow(t,2)/(2*torch.pow(s,2))))/torch.pow(s,5) - (torch.pow(t,3)*torch.exp(-torch.pow(t,2)/(2*torch.pow(s,2))))/torch.pow(s,7)

        if reverse == True:
            y = (torch.pow(t,3)*torch.exp(-torch.pow(t,2)/(2*torch.pow(s,2))))/torch.pow(s,7) - (3*t*torch.exp(-torch.pow(t,2)/(2*torch.pow(s,2))))/torch.pow(s,5)
