...
class VehicleBatch():
    ...
    def __init__(
        ...
        if init_pos is not None:
            init_pos = torch.as_tensor(init_pos, dtype=torch.float32, device=self.device)
            
        if init_orientation is not None:
            init_orientation = torch.stack(init_orientation, dim=0).to(self.device)

        # Spawn the batch of vehicle's in the world's stage
        self._spawn_batch_grid_cloner(spacing=spacing, init_pos=init_pos, init_orientation=init_orientation)
    ...
    def _spawn_batch_grid_cloner(self, spacing, init_pos=None, init_orientation=None):
        ...
        #xformable.SetTranslate(Gf.Vec3d(float(0.0), float(0.0), float(0.3)))
        ...
        if init_pos is not None:
            ...
            if init_orientation is not None:
                cloner.clone(source_prim_path=f"{self._stage_prefix}_0", prim_paths=target_paths, position_offsets=init_pos, orientation_offsets=init_orientation, replicate_physics=True, copy_from_source=True, base_env_path="/World", root_path=f"{self._stage_prefix}_")
            else:
                cloner.clone(source_prim_path=f"{self._stage_prefix}_0", prim_paths=target_paths, position_offsets=init_pos, replicate_physics=True, copy_from_source=True, base_env_path="/World", root_path=f"{self._stage_prefix}_")
        
        else:
            cloner.clone(source_prim_path=f"{self._stage_prefix}_0", prim_paths=target_paths, replicate_physics=True, copy_from_source=True, base_env_path="/World", root_path=f"{self._stage_prefix}_")
        ...
        # Disable collisions between the cloned vehicles (optional, but can improve performance if we are not interested in simulating collisions between vehicles).
        #cloner.filter_collisions(physicsscene_path="/physicsScene", prim_paths=target_paths, collision_root_path=f"{self._stage_prefix}_collision_groups",global_paths=[])
